...
class ClubRelicPlayer:
    """Player chasing a moving relic."""
    ...
    def __init__(self, follower_data: dict, position: Tuple[float, float]):
        ...
        self.move_speed = float(getattr(config, "CLUB_RELIC_MOVE_SPEED", 100.0))
        self.turn_rate = float(getattr(config, "CLUB_RELIC_TURN_RATE", 0.25))

    def update(self, dt: float, arena, relic_pos: Tuple[float, float]):
        if not self.alive:
            return

        dx = relic_pos[0] - self.x
        dy = relic_pos[1] - self.y
