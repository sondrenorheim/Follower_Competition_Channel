...
class SuperFollowerBrosRenderer(RendererTemplate):
    ...
    def __init__(self, screen: pygame.Surface):
        ...
        y_shift = int(getattr(config, "SUPER_FOLLOWER_BROS_1_2_RENDER_Y_OFFSET", 0))
        self.game_left = arena_x
        self.game_top = arena_y + y_shift
        self.game_right = arena_x + arena_w
        self.game_bottom = arena_y + arena_h + y_shift
        ...
        self.background_path = "assets/super_follower_bros_1_2/level_2.png"
    ...
    def _get_tileset_frames(self, level):
        ...
        # Use underground palette row so blocks align with the 1-2 background tiles.
        brick_frame = slice_frame(16, 0, 16, 16)
        open_frame = slice_frame(432, 112, 16, 16)
        question_frames = [
            slice_frame(384, 96, 16, 16),
            slice_frame(400, 96, 16, 16),
            slice_frame(384, 96, 16, 16),
        ]
    ...
    def _get_moving_platform_frames(self, level):
        ...
        if scale <= 0:
            return None
        if self._moving_platform_frames is not None and self._moving_platform_scale == scale:
        ...
        # Use brick deck tiles for 1-2 lifts (avoids pipe glyph fragments).
        left = slice_frame(16, 0, 16, 16)
        middle = slice_frame(16, 0, 16, 16)
        right = slice_frame(16, 0, 16, 16)
        self._moving_platform_frames = {
            "left": left,
            "middle": middle,
            "right": right,
        }
        self._moving_platform_scale = scale
        return self._moving_platform_frames
    ...
    def _draw_game_area(self, players, game_state: dict):
        ...
        if level:
            ...
            self._draw_moving_platforms(level, camera_x)
            ...
            self._draw_checkpoint_flags(level, camera_x, game_state.get("checkpoints", []))
    ...
    def _draw_checkpoint_flags(self, level, camera_x: float, checkpoints):
        if level is None or not checkpoints:
            return
        margin = int(getattr(config, "SUPER_FOLLOWER_BROS_RENDER_MARGIN", 140))
        pole_color = getattr(config, "SUPER_FOLLOWER_BROS_CHECKPOINT_POLE_COLOR", (226, 230, 236))
        flag_active = getattr(config, "SUPER_FOLLOWER_BROS_CHECKPOINT_FLAG_ACTIVE_COLOR", (84, 210, 112))
        flag_inactive = getattr(config, "SUPER_FOLLOWER_BROS_CHECKPOINT_FLAG_INACTIVE_COLOR", (130, 130, 138))
        pole_width = float(getattr(config, "SUPER_FOLLOWER_BROS_CHECKPOINT_POLE_WIDTH", 5.0)) * float(getattr(level, "scale", 1.0))
        pole_height = float(getattr(config, "SUPER_FOLLOWER_BROS_CHECKPOINT_FLAG_HEIGHT", 120.0)) * float(getattr(level, "scale", 1.0))
        flag_width = float(getattr(config, "SUPER_FOLLOWER_BROS_CHECKPOINT_FLAG_WIDTH", 20.0)) * float(getattr(level, "scale", 1.0))
        flag_height = float(getattr(config, "SUPER_FOLLOWER_BROS_CHECKPOINT_FLAG_TRI_HEIGHT", 14.0)) * float(getattr(level, "scale", 1.0))
        block_height = float(getattr(level, "brick_size", 0.0))
        y_offset = float(getattr(config, "SUPER_FOLLOWER_BROS_CHECKPOINT_Y_OFFSET", 10.0)) * float(
            getattr(level, "scale", 1.0)
        )
        base_y = self.game_top + level.ground_y - block_height + y_offset

        for idx, checkpoint in enumerate(checkpoints, start=1):
            flag_x = float(checkpoint.get("spawn_x", checkpoint.get("x", 0.0)))
            pole_x = self.game_left + flag_x - camera_x
            if pole_x < self.game_left - margin or pole_x > self.game_right + margin:
                continue

            top_y = base_y - pole_height
            color = flag_active if checkpoint.get("active") else flag_inactive

            # Wide translucent beam so checkpoint is easy to spot during gameplay.
            beam_color = (84, 210, 112, 46) if checkpoint.get("active") else (180, 180, 190, 36)
            beam = pygame.Surface((max(8, int(round(pole_width * 2.6))), int(round(self.game_bottom - self.game_top))), pygame.SRCALPHA)
            beam.fill(beam_color)
            beam_rect = beam.get_rect(midtop=(int(round(pole_x)), int(round(self.game_top))))
            self.screen.blit(beam, beam_rect)

            pole_rect = pygame.Rect(
                int(round(pole_x - pole_width * 0.5)),
                int(round(top_y)),
                max(1, int(round(pole_width))),
                max(1, int(round(pole_height))),
            )
            pygame.draw.rect(self.screen, pole_color, pole_rect)
            pygame.draw.rect(self.screen, (18, 22, 30), pole_rect, 1)

            flag_y = top_y + pole_height * 0.18
            tip_x = pole_rect.left - int(round(flag_width))
            points = [
                (int(round(pole_rect.left)), int(round(flag_y))),
                (tip_x, int(round(flag_y + flag_height * 0.5))),
                (int(round(pole_rect.left)), int(round(flag_y + flag_height))),
            ]
            pygame.draw.polygon(self.screen, color, points)
            pygame.draw.polygon(self.screen, (20, 20, 24), points, 2)

            label = f"CP{idx}"
            label_surface = self.font_small.render(label, True, (245, 245, 250))
            label_bg = label_surface.get_rect(
                center=(int(round(pole_x)), int(round(top_y - 10)))
            )
            label_bg.inflate_ip(8, 4)
            pygame.draw.rect(self.screen, (20, 22, 30), label_bg, border_radius=3)
            pygame.draw.rect(self.screen, (90, 94, 110), label_bg, 1, border_radius=3)
            label_rect = label_surface.get_rect(center=label_bg.center)
            self.screen.blit(label_surface, label_rect)

    def _draw_flag(self, level, camera_x: float):
        # Level 1-2 ends at a goal pipe; no flagpole rendering here.
        return
    ...
    def _draw_moving_platforms(self, level, camera_x: float):
        platforms = getattr(level, "moving_platforms", None)
        if not platforms:
            return

        margin = int(getattr(config, "SUPER_FOLLOWER_BROS_RENDER_MARGIN", 140))
        border_color = getattr(config, "SUPER_FOLLOWER_BROS_BLOCK_BORDER", (60, 36, 18))
        fallback_fill = getattr(config, "SUPER_FOLLOWER_BROS_PIPE_COLOR", (0, 168, 0))
        frames = self._get_moving_platform_frames(level)
        ...
        for platform in platforms:
            ...
            if not frames:
                pygame.draw.rect(self.screen, fallback_fill, screen_rect)
                pygame.draw.rect(self.screen, border_color, screen_rect, 2)
                continue

            left = frames.get("left")
            middle = frames.get("middle")
            ...
            tile_h = middle.get_height()
            deck_y = screen_rect.top + max(0, (screen_rect.height - tile_h) // 2)
            left_x = screen_rect.left
            right_x = screen_rect.right - right.get_width()
            self.screen.blit(left, (left_x, deck_y))
            cursor = left_x + left.get_width()
            while cursor < right_x:
                self.screen.blit(middle, (cursor, deck_y))
                cursor += middle.get_width()
            self.screen.blit(right, (right_x, deck_y))
            # Keep a crisp top edge so lifts are readable over the dark background.
            pygame.draw.line(
                self.screen,
                border_color,
                (screen_rect.left, deck_y),
                (screen_rect.right - 1, deck_y),
                1,
            )
    ...
    def _draw_minimap(self, players, game_state: dict):
        ...
        self._draw_minimap_checkpoints(game_state, map_x, map_y, map_w, map_h, level_width)
    ...
    def _draw_minimap_checkpoints(
        ...
        if not checkpoints:
            return
        line_active = getattr(config, "SUPER_FOLLOWER_BROS_CHECKPOINT_FLAG_ACTIVE_COLOR", (84, 210, 112))
        line_inactive = getattr(config, "SUPER_FOLLOWER_BROS_CHECKPOINT_FLAG_INACTIVE_COLOR", (130, 130, 138))
        top = map_y + 2
        bottom = map_y + map_h - 2

        for idx, checkpoint in enumerate(checkpoints, start=1):
            cp_x = float(checkpoint.get("spawn_x", checkpoint.get("x", 0.0)))
            marker_x = map_x + int((max(0.0, min(level_width, cp_x)) / level_width) * map_w)
            color = line_active if checkpoint.get("active") else line_inactive
            pygame.draw.line(self.screen, color, (marker_x, top), (marker_x, bottom), 2)
            pygame.draw.circle(self.screen, color, (marker_x, top + 4), 3)
            pygame.draw.circle(self.screen, color, (marker_x, bottom - 4), 3)
            label = self.font_tiny.render(f"CP{idx}", True, (245, 245, 250))
            label_rect = label.get_rect(center=(marker_x, top + 14))
            bg = label_rect.inflate(6, 2)
            pygame.draw.rect(self.screen, (10, 12, 20), bg, border_radius=2)
            pygame.draw.rect(self.screen, color, bg, 1, border_radius=2)
    ...
    def _draw_game_ui(self, players, game_state: dict):
        ...
        world_label = getattr(config, "SUPER_FOLLOWER_BROS_1_2_WORLD_LABEL", "1-2")
        ...
        checkpoints = game_state.get("checkpoints", [])
        progress_x = float(game_state.get("checkpoint_progress_x", 0.0))
        unlocked_index = int(game_state.get("checkpoint_unlocked_index", -1))
        if unlocked_index < -1:
            unlocked_index = -1
        if unlocked_index >= len(checkpoints):
            unlocked_index = len(checkpoints) - 1

        active_idx = unlocked_index + 1
        respawn_label = "RESPAWN START" if active_idx <= 0 else f"RESPAWN CP{active_idx}"
        respawn_color = (180, 184, 194) if active_idx <= 0 else (96, 232, 128)
        respawn_surface = self.font_tiny.render(respawn_label, True, respawn_color)
        self.screen.blit(respawn_surface, (panel_x, panel_y + 37))

        if not checkpoints:
            next_label = "NEXT CHECKPOINT: NONE"
            next_color = (180, 184, 194)
            next_surface = self.font_tiny.render(next_label, True, next_color)
            self.screen.blit(next_surface, (panel_x, panel_y + 52))
            return

        next_checkpoint = None
        for idx, checkpoint in enumerate(checkpoints, start=1):
            cp_x = float(checkpoint.get("x", 0.0))
            if cp_x > progress_x:
                next_checkpoint = (idx, cp_x)
                break

        if next_checkpoint is None:
            next_label = "NEXT CHECKPOINT: COMPLETE"
            next_color = (96, 232, 128)
        else:
            cp_idx, cp_x = next_checkpoint
            distance = max(0, int(round(cp_x - progress_x)))
            next_label = f"NEXT CP{cp_idx}: +{distance}px"
            next_color = (212, 216, 226)
        next_surface = self.font_tiny.render(next_label, True, next_color)
        self.screen.blit(next_surface, (panel_x, panel_y + 52))

        status_label = f"CHECKPOINTS: {max(0, active_idx)}/{len(checkpoints)}"
        status_surface = self.font_tiny.render(status_label, True, (180, 184, 194))
        self.screen.blit(status_surface, (panel_x, panel_y + 67))
