...
class ClubDuelPlayer:
    """Player that moves toward duel targets."""
    ...
    def __init__(self, follower_data: dict, position: Tuple[float, float]):
        ...
        self.move_speed = float(getattr(config, "CLUB_DUEL_MOVE_SPEED", 110.0))
        self.turn_rate = float(getattr(config, "CLUB_DUEL_TURN_RATE", 0.25))
        self.skill = random.uniform(0.8, 1.2)
