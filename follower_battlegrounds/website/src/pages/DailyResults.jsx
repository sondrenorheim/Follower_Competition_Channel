...
import { loadDay, loadIndex, loadDayAggregate, loadGame, loadGamePreview, normalizeResultPlatform } from '../utils/dataLoader';
...
export default function DailyResults() {
  ...
  useEffect(() => {
    setSearchQuery('');
    setCurrentPage(1);
  }, [selectedGameType, selectedPlatform]);
  ...
  useEffect(() => {
    ...
    async function loadSelection() {
      ...
      let game = null;
      if (selectedGameType === 'all') {
        game = await loadDayAggregate(selectedDayNumber, { preview: true, platform: selectedPlatform });
      } else {
        const dayData = await loadDay(selectedDayNumber, { platform: selectedPlatform });
        if (dayData && dayData.games) {
          ...
          if (matches.length > 0) {
            ...
            game = await loadGamePreview(latestGame.game_id, { platform: selectedPlatform });
          }
        }
      }
  ...
  }, [selectedDayNumber, selectedGameType, selectedPlatform, indexLoaded]);
  ...
  const loadFullResults = React.useCallback(async () => {
    ...
    try {
      const fullGame = selectedGame.game_type === 'all'
        ? await loadDayAggregate(selectedGame.day_number, { preview: false, platform: selectedPlatform })
        : await loadGame(selectedGame.game_id, { platform: selectedPlatform });
      if (fullGame) {
