"""
Fighter Arena Module
Handles the rectangular arena for Fighter Arena game mode
"""
...
class FighterArena:
    """
    Rectangular arena for the Fighter Arena game mode
    Unlike Battle Royale, this arena doesn't shrink
    """
    ...
    def __init__(self):
        ...
        print(f"Fighter Arena initialized: ({self.x}, {self.y}) - {self.width}x{self.height}")
    ...
    def update(self, dt: float) -> bool:
        ...
        Returns:
            False (no shrinking in Fighter Arena)
        """
        # Fighter Arena doesn't shrink, but this could be extended
        # for power-up spawning, hazards, etc.
        return False

    def __repr__(self):
        return f"FighterArena({self.x}, {self.y}, {self.width}x{self.height})"
