...
class MathDropPlayer:
    def __init__(self, follower_data: dict, position: Tuple[float, float]):
        ...
        self.vx = 0.0
        self.vy = 0.0
        self.radius = float(getattr(config, "MATH_DROP_PLAYER_RADIUS", 15))
        ...
        self.target_refresh_range = getattr(config, "MATH_DROP_TARGET_REFRESH", (0.6, 1.4))
        self.target_refresh = random.uniform(*self.target_refresh_range)

        self.move_speed = float(getattr(config, "MATH_DROP_MOVE_SPEED", 80.0))
        self.turn_rate = float(getattr(config, "MATH_DROP_TURN_RATE", 0.18))
        self.direction_jitter = float(getattr(config, "MATH_DROP_DIRECTION_JITTER", 0.2))

        self.wander_speed = float(getattr(config, "MATH_DROP_WANDER_SPEED", 35.0))
        self.wander_interval_range = getattr(config, "MATH_DROP_WANDER_INTERVAL", (0.8, 1.6))
        self.wander_interval = random.uniform(*self.wander_interval_range)
        self.wander_timer = 0.0
        self.wander_angle = random.uniform(0.0, 2 * math.pi)

        self.fall_speed = float(getattr(config, "MATH_DROP_FALL_SPEED", 260.0))

    def assign_choice(self, choice: int, arena):
        self.choice = choice
    ...
    def update(self, dt: float, arena, phase: str):
        ...
        # Let players travel to the upper squares during selection/result.
        # Keep them within the overall arena bounds to avoid hard teleports.
        if hasattr(arena, "clamp_position"):
            self.x, self.y = arena.clamp_position(self.x, self.y, self.radius)
        else:
            self.x, self.y = arena.clamp_to_lower_half(self.x, self.y, self.radius)
    ...
    def _pick_new_target(self, arena):
        if self.choice is not None and hasattr(arena, "get_random_position_in_choice"):
            self.target_x, self.target_y = arena.get_random_position_in_choice(
                self.choice,
                margin=self.radius + 4,
            )
        else:
            self.target_x, self.target_y = arena.get_random_position_in_lower_half(
                margin=self.radius + 4,
            )
