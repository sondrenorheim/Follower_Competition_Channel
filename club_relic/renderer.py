...
class ClubRelicRenderer(RendererTemplate):
    """Renderer for the Club Relic game."""
    ...
    def __init__(self, screen: pygame.Surface):
        ...
        self.day_counter_offset = int(getattr(config, "CLUB_RELIC_DAY_COUNTER_OFFSET", 16))

        self.arena_fill = (205, 205, 210)
        self.arena_border = (40, 40, 40)
        self.relic_color = (255, 210, 120)
        self.relic_glow = (255, 235, 180)
    ...
    def _draw_game_area(self, players: List, game_state: dict):
        ...
        relic_pos = game_state.get("relic_pos")
        if relic_pos:
            elapsed = float(game_state.get("elapsed_time", 0.0))
            pulse = 4 + 3 * math.sin(elapsed * 4.0)
            glow_radius = int(getattr(config, "CLUB_RELIC_RELIC_RADIUS", 16) + pulse + 6)
            pygame.draw.circle(self.screen, self.relic_glow, (int(relic_pos[0]), int(relic_pos[1])), glow_radius)
            pygame.draw.circle(
                self.screen,
                self.relic_color,
                (int(relic_pos[0]), int(relic_pos[1])),
                int(getattr(config, "CLUB_RELIC_RELIC_RADIUS", 16)),
            )
            pygame.draw.circle(
                self.screen,
                self.arena_border,
    ...
    def _draw_players(self, players: List, game_state: dict = None):
        ...
        if game_state:
            elapsed = float(game_state.get("elapsed_time", 0.0))
        fade_duration = float(getattr(config, "CLUB_RELIC_FADE_DURATION", config.FADE_DURATION))
    ...
    def _draw_game_ui(self, players: List, game_state: dict):
        ...
        timer_text = self.font_status.render(f"Hunt: {time_left:.1f}s", True, (255, 255, 255))
        ...
        if last_eliminated:
            ...
            self.screen.blit(elim_text, (panel_x + 12, panel_y + 70))
